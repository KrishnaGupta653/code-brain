...
export class SQLiteStorage {
  ...
  getProjectId(projectRoot: string): string {
    return stableId('project', projectRoot);
  }

  saveProject(metadata: ProjectMetadata): string {
    try {
      const projectId = this.getProjectId(metadata.root);
      const now = Date.now();
  ...
  saveFileHashes(projectRoot: string, files: Array<{ path: string; hash: string; language: string; size: number }>): void {
    const projectId = this.getProjectId(projectRoot);
    const now = Date.now();
    ...
    const tx = this.db.transaction(() => {
      for (const file of files) {
        upsert.run(projectId, file.path, file.language, file.hash, file.size, now, now);
      }
    });
  ...
  removeMissingFileHashes(projectRoot: string, currentPaths: string[]): void {
    ...
    const del = this.db.prepare('DELETE FROM files WHERE project_id = ? AND path = ?');
    const tx = this.db.transaction(() => {
      for (const row of rows) {
  ...
  replaceGraph(projectRoot: string, graph: GraphModel): void {
    ...
    if (!project) {
      throw new StorageError('Project not initialized. Run code-brain init first.');
    }
    ...
    const tx = this.db.transaction(() => {
      this.db.prepare('DELETE FROM provenance WHERE project_id = ?').run(projectId);
      ...
      for (const node of nodes) {
        ...
        this.insertProvenanceSpans(insertProv, projectId, node.id, node.provenance);
      }
      ...
      for (const edge of edges) {
        ...
        this.insertProvenanceSpans(insertProv, projectId, edge.id, edge.provenance);
      }
    });
  ...
  loadGraph(projectRoot: string): GraphModel {
    ...
    for (const row of nodes) {
      ...
      node.provenance = this.getProvenanceFor(row.id, projectId, row.created_at, row.updated_at);
      graph.addNode(node);
    }
    ...
    for (const row of edges) {
      ...
      edge.provenance = this.getProvenanceFor(row.id, projectId, row.created_at, row.updated_at);
      graph.addEdge(edge);
    }
  ...
  saveRankingScores(projectRoot: string, scores: RankingScore[]): void {
    const projectId = this.getProjectId(projectRoot);
    const now = Date.now();
    const del = this.db.prepare('DELETE FROM ranking_scores WHERE project_id = ?');
  ...
  private insertProvenanceSpans(
    ...
    const sourceSpans = provenance.source.length > 0
      ? provenance.source
      : [
          {
            file: '',
    ...
    for (const source of sourceSpans) {
      insertProv.run(
        stableId('prov', projectId, nodeId, source.file || '', source.startLine, source.startCol, index++),
        projectId,
        nodeId,
        provenance.type,
        source.file || '',
  ...
  private getSymbolCount(projectId: string): number {
    const row = this.db
      .prepare("SELECT COUNT(*) as count FROM nodes WHERE project_id = ? AND type NOT IN ('file', 'project')")
      .get(projectId) as { count: number };
    return row?.count || 0;
  }
